...
import { Button, IconButton, CardMedia, CardContent, Typography } from '@material-ui/core'
...
const ProductCard = props => {
  ...
  let imageUrl = item?.media[0]?.formats?.small?.url
        ...
        <Link href="/products/[url]" as={`/products/${item.url}`}>
          <a className={classes.cardContentInfo}>
            ...
            <div className={classes.detailsContainer}>
              ...
              {!item?.direct_shipping && (
                <div className={classes.shippingMessage}>{t('eligible-free-shipping')}</div>
              )}
              {item?.direct_shipping && (
                <div className={classes.shippingMessage}> {t('free-shipping-included')}</div>
              )}
            </div>
          </a>
        </Link>
