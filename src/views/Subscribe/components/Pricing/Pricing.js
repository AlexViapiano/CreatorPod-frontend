  ...
  useEffect(() => {
    ...
    getPrices().then(res => {
      var foundMonthlyPrice = res?.data?.find(
        price =>
          price.id == 'price_1J7jCvLQ3xHrCGemFDAnhR6i' ||
          price.id == 'price_1ITXZGLQ3xHrCGemBSuo82bL'
      )

      // AUTO SELECT MONTHLY PRICE
      if (foundMonthlyPrice) setPrice(foundMonthlyPrice.id)
      // OTHERWISE SELECT FIRST PLAN IN LIST
      else res?.data[0] && setPrice(res?.data[0].id)
              ...
              <div className={classes.priceContainer}>
                ...
                >
                  <div className={classes.subscriptionContainer}>
                    ...
                    <span>
                      {t('billed-every')} {_price?.recurring?.interval}
                    </span>
                    <hr />
                    <br />
