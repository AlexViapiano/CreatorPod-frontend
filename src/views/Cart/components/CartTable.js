...
const CartTable = props => {
  ...
  useEffect(() => {
    if (checkoutSessionId == null && checkoutSessionCreated == false) {
      ...
      createCheckoutSession(session_cart)
    }
  }, [])
  ...
  const initiateCheckout = async () => {
    ...
    updateCheckoutSession(checkoutSessionId, { initiate_checkout: true })
  }

  const basic_cart = cart.filter(product => !product.direct_shipping)
  const direct_cart = cart.filter(product => product.direct_shipping)
  ...
  if (shipping_fee_included) order_subtotal = order_subtotal + 10
...
const mapDispatchToProps = dispatch => ({
  ...
  createCheckoutSession: cart => {
    return dispatch(createCheckoutSession(cart))
  },
  updateCheckoutSession: (checkoutSessionId, data) => {
    return dispatch(updateCheckoutSession(checkoutSessionId, data))
  },
})
