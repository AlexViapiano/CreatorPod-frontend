...
export const updateCheckoutSession = (checkoutSessionId, data, reset) => {
  return dispatch => {
    dispatch({ type: actionTypes.REQUEST_REQUEST_SET_SESSION })
...
export const createSubscribeSession = (userId, metadata) => {
  return dispatch => {
    dispatch({ type: actionTypes.REQUEST_SET_SUBSCRIBE_SESSION })
    return fetch(`${API_URL}/session-subscribes`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
